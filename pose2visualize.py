...
class R2GaussianSceneRenderer:
    ...
    def get_eye_transform(self, charuco_tf, charuco_center, eye_position: str = "top"):
        if eye_position == "top":
    ...
    def plot_board_transform(self, charuco_tf, charuco_center, colors = ('r', 'g', 'b'), eye_position: str = "top"):
        ...
        eye_tf= self.get_camera_transform(charuco_tf, charuco_center, eye_position)

        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(*charuco_center, color='orange', s=50, label='ChArUco Center')

        def plot_frame(ax, tf, charuco_center, label, color):
            origin = tf[:3, 3]
        ...
        plot_frame(ax, charuco_tf, charuco_center, 'Charuco', colors)
        plot_frame(ax, camera_tf, charuco_center, 'Camera', colors)
        plot_frame(ax, eye_tf, charuco_center, 'Eye', colors)
        ...
        # アスペクト比を揃える
        points = np.vstack((charuco_center, camera_tf[:3, 3], eye_tf[:3, 3]))
    ...
    def cut_gaussians_by_plane(gaussians, charuco_tf, d=0.1):
        with torch.no_grad():
            origin = charuco_tf[:3, 3]
    ...
    def cut_gaussians_beyond_plane(gaussians, charuco_tf, d=0.1):
        with torch.no_grad():
            origin = charuco_tf[:3, 3]
            normal = charuco_tf[:3, 2]
            # 正規化された法線ベクトル
    ...
    def render_gaussians(self, gaussians, charuco_tf, charuco_center, d=0.1, eye_position: str = "top", cut_method: str="by_plane"):
        view = self.get_eye_transform(charuco_tf, charuco_center, eye_position)

        cut_gaussians = copy.deepcopy(gaussians)
        ...
        rendering = render(view, gaussians, self.pipeline)["render"][0].detach().cpu().numpy()
    ...
    if __name__ == "__main__":
        ...
        rendering, rendering_cut = renderer.render_gaussians(gaussians, charuco_tf, charuco_center, d, eye_position, cut_method)
